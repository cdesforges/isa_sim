...
numbits = 2 ** 32  # we increased our register and memory size to 32-bit to handle the larger numbers generated by a fib-machine

# testing

print("\nWelcome to the ISA simulator! - Designed by <Chandler, Marcus and Aryavardhan>")
    ...
    def write_register(self, register, register_value):
        if register in self.registers:
            if register == 'R0':
                ...
                self.registers[register] = register_value % numbits
        else:
...
class DataMemory:
    ...
    def write_data(self, address, data):
        if address < 0 or address > 255:
            ...
            sys.exit(-1)
        self.data_memory[address] = data % numbits
...
def executeInstruction(opcode, op1, op2, op3=None):
    ...
    if opcode == "NOT":
        ...
        return
    if opcode == "INC":
        registerFile.write_register(op1, registerFile.read_register(op1) + int(op2))
        return  # we made an incrementer function to keep track of how many itterations we have gone through
    if opcode == "DEC":
        registerFile.write_register(op1, registerFile.read_register(op1) - int(op2))
        return  # we also made a decrementer
    if opcode == "MOV":
        registerFile.write_register(op1, registerFile.read_register(op2))
        return  # we made a move function
    ...
    if opcode == "SD":
        ...
        print(f"saving {registerFile.read_register(op1)} to address {registerFile.read_register(op2)}")
    ...
    if opcode == "JLT":
        if registerFile.read_register(op2) < registerFile.read_register(op3):
            ...
            program_counter += 1
    if opcode == "JGT":
        if registerFile.read_register(op2) > int(op3):
            print(f"checkpoint: {op1}")
            program_counter = int(op1)
            print(f"going to line = {op1}")
            return
        else:
            program_counter += 1
    if opcode == "NOP":
...
while cycle < max_cycles and not program_complete:
    ...
    if op_code != "JR" and op_code != "JEQ" and op_code != "JLT" and op_code != "JGT":
        program_counter += 1
